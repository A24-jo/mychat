...
import MobileFrients from "@/components/MobileFrients";
...
export default function HomeLayout({ children }) {
  ...
  const MobileSelect = useSelector(state => state.chat.mobileSelect)
  ...
  return (
    <div className="h-screen flex dark:bg-gray-900 dark:text-white">
      {children}
      {MobileSelect === 2 && <ModalPerfil mobile={MobileSelect} />}
      {MobileSelect === 3 && < MobileContacts />}
      {MobileSelect === 4 && < MobileFrients />}
      {MobileSelect === 1 && <div className="w-full h-[100vh] flex">
          ...
          <div className=" flex items-center justify-between  bg-gray-200 p-4 border-b border-gray-300 dark:border-gray-600 dark:bg-gray-700">
            <div className="flex items-center">
              ...
              <div className="w-12 h-12 rounded-full flex items-center justify-center text-white text-xl font-semibold"
                style={{ backgroundColor: selectedUser?.avatarcolor? selectedUser?.avatarcolor : "#0e7ad3" }}
              >
                {selectedUser.name[0]}
              </div>
